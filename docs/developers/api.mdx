...
<Callout type="info">
  **Common integration events**
  ...
  - Block / furniture events: `events.noteblock.*`, `events.stringblock.*`, `events.chorusblock.*`, `events.shapedblock.*`, `events.furniture.*`. The `events.shapedblock.*` family currently contains only `OraxenShapedBlockBreakEvent`; there are no shaped-block damage, interact, or place variants.
</Callout>

...
The `Oraxen Item ID` Skript type represents an Oraxen item identifier and can be used by the item expressions and conditions below. Event syntax also accepts alternate phrasings such as `on oraxen block break of <id>` and `on break of oraxen block <id>` where supported.
...
Remove an Oraxen block at a location. For block types, this normally spawns the block's default drop; it does not guarantee a no-drop removal.
